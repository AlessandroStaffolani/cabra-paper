...
from cabra import logger, ROOT_DIR
...
from cabra.run.remote.redis_constants import REDIS_RUNS_QUEUE, REDIS_MANAGEMENT_KEY, \
    REDIS_MANAGEMENT_EVENT_KEY, REDIS_ALL_WORKERS_CODE, REDIS_ALL_RUN_WORKERS_CODE, FAILED_RUNS_QUEUE, \
...
MPI_RUNNER_SCRIPT = 'mpi_runner.py'
...
def schedule_run(
    ...
    runner = RemoteRunner(run_code)
    runner.run()
    return p_index


def schedule_mpi_run(run_code: str, n_workers: int, **kwargs):
    mpi_fork(n=n_workers, run_code=run_code, script_name=os.path.join(ROOT_DIR, MPI_RUNNER_SCRIPT), exit_on_end=False)
...
class Worker:
    ...
    def __init__(
        ...
        if self.use_mpi:
            TYPE_WORKER_MAPPING[WorkerType.RunWorker]['schedule_func'] = schedule_mpi_run
        self.processes = processes if not self.use_mpi else 0
        self.mpi_workers = processes
        ...
        self.uses_sub_processes: bool = True if self.processes > 0 else False
    ...
    def _handle_run_completed(self, result):
        if self.uses_sub_processes:
            ...
            if self.max_runs is not None and self.completed_runs >= self.max_runs:
                self.stop = True
                logger.info(
                    f'Worker {self.worker_code} completed '
                    f'{self.completed_runs} runs and it has been asked to stop')

    def _handle_management_messages(self, message):
        try:
            # STOP ALL MESSAGE:
            # publish worker_management_event '{"type": "STOP", "worker_code": "workers-all"}'
            ...
            if self.uses_sub_processes:
                if process_index is not None:
                    ...
                    self.pool.apply_async(
                        func=func,
                        kwds={
                            'run_code': f'{run_code}_{self.worker_code}',
                            'p_index': process_index,
                            'services_endpoints': self.services_endpoints,
                            'n_workers': self.mpi_workers,
                        },
                ...
                try:
                    func(run_code=f'{run_code}_{self.worker_code}', p_index=process_index,
                         services_endpoints=self.services_endpoints, n_workers=self.mpi_workers)
                    self.completed_runs += 1
                    if self.max_runs is not None and self.completed_runs >= self.max_runs:
                        self.stop = True
                        logger.info(
