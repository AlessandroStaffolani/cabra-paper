...
from cabra.common.remote.mongo import MongoWrapper
from cabra.run import RunStatus
from cabra.run.multi_run_config import AGENT_WITH_VALIDATION
from cabra.run.config import MultiRunParamConfig
...
class MongoRunWrapper(MongoWrapper):
    ...
    def __init__(
        ...
        super(MongoRunWrapper, self).__init__(host, port)
        self.run_db = os.getenv('MONGO_DB') if os.getenv('MONGO_DB') is not None else run_db
        ...
        self.mutl_run_db = os.getenv('MONGO_DB') if os.getenv('MONGO_DB') is not None else multi_run_db
    ...
    def add_scheduled_run(
            ...
            evaluation_runs: Optional[List[ObjectId]] = None,
        ...
        doc = {
            ...
            'evaluation_runs': [] if evaluation_runs is None else evaluation_runs,
            'created_at': now,
            'last_modified': now,
        }
    ...
    def update_scheduled_run(
        ...
        if run_stats is not None:
            for key, val in run_stats.items():
                update_obj[key] = val
        if evaluation_runs is not None and isinstance(evaluation_runs, list):
            update_obj['evaluation_runs'] = evaluation_runs
        if len(update_obj) > 0:
    ...
    def save_multi_run_single_run(
        ...
        for param in run_params:
            doc[f'hp/{param.key_short}'] = param.value if param.value is not None else 'None'
        agent_type = AgentType(run_data['config']['environment']['agent']['type'])
        self.set_baselines_hp(agent_type, doc, run_data)
    ...
    def set_baselines_hp(self, agent_type: AgentType, doc: dict, run_data: dict):
        if agent_type in [AgentType.ConstrainedGreedy, AgentType.ConstrainedRandom]:
            doc['hp/critical_threshold'] = run_data['config']['environment']['constrained_space']['critical_threshold']
            doc['hp/max_distance'] = run_data['config']['environment']['constrained_space']['max_distance']
            doc['hp/zone_max_distance'] = run_data['config']['environment']['constrained_space']['zone_max_distance']
            doc['hp/zones_filtered_size'] = run_data['config']['environment']['constrained_space'][
                'zones_filtered_size']
