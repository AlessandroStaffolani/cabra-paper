...
from cabra.common.filesystem import get_absolute_path
...
class AbstractConfig:
    ...
    def __init__(self, config_object_name=None, root_dir=None, **configs_to_override):
        ...
        self.base_data_dir: str = get_absolute_path(os.getenv('DATA_BASE_DIR'))
        self.logs_base_dir: str = get_absolute_path(os.getenv('LOGS_BASE_DIR'))
        self.export_exclude = ['export_exclude', '_name', 'root_dir', 'base_data_dir', 'logs_base_dir']
    ...
    def set_enum_from_string(self, field_name: str, enum_class):
        current_value = getattr(self, field_name)
        if isinstance(current_value, str):
            try:
                setattr(self, field_name, enum_class(current_value))
            except Exception:
                raise ConfigValueError(field_name, current_value, module=self.name(),
                                       extra_msg=f'Possible values are: {enum_class.list()}')
    ...
    def export(self, save_path=None, mode: ExportMode = ExportMode.DICT) -> None or dict or str:
...
class LoggerConfig(AbstractConfig):
    ...
    def __init__(self, config_object_name: str, log_folder: str = 'bs_repositioning', **configs_to_override):
    ...
    def get_handlers(self):
        ...
        for handler in self.handlers:
            if handler['type'] == 'file':
                file_handler = deepcopy(handler)
            else:
                final_handlers.append(deepcopy(handler))
        if file_handler is not None:
            file_handler['parameters']['log_basepath'] = self.logs_base_dir
        final_handlers.append(file_handler)
