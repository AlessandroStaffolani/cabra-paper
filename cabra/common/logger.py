...
from cabra.common.mpi.mpi_tools import proc_id
...
FORMATTER = Iso8601UTCTimeFormatter(
    f"%(asctime)s | %(name)s "
    f"| Rank: {proc_id()} | (%(processName)-10s) | (%(threadName)-10s) | %(levelname)s | %(message)s",
    time_zone='local')
...
class BaseLogger(logging.Logger):
    ...
    def _set_handler(self, logger_config):
        if len(self.handlers) == 0:
            for handler_config in logger_config.get_handlers():
                if handler_config['type'] == 'console':
                    handler = get_console_handler()
...
def get_logger(logger_config=None) -> logging.Logger:
